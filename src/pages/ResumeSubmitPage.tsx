...
export default function ResumeSubmitPage() {
  ...
  function onGeneratePdf(data: ResumeSubmitFormData) {
    try {
      const candidateName =
        `${data.firstName} ${data.lastName}`.trim() || "Candidate";
      const title = `Resume PDF: ${candidateName}`;
  ...
  async function onSubmitToServer(data: ResumeSubmitFormData) {
    ...
    const candidateName =
      `${data.firstName} ${data.lastName}`.trim() || "Candidate";
    const jobTitle = data.jobTarget || "Job target";

    const resumeIdFromUpload =
      data.email?.trim() ||
      `${data.firstName || "first"}-${data.lastName || "last"}`.trim();
  ...
  return (
    <section className="space-y-4">
      ...
      >
        ...
        <div className="mt-4 rounded-md border border-gray-200 bg-gray-50 p-3 text-xs text-gray-600 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300">
          Изображение/фото в этой задаче не загружается. При необходимости можно
          расширить форму, чтобы добавлять/хранить фото и отображать его в
          шаблоне PDF.
        </div>
        ...
        <div className="mt-4 flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
          ...
          <button
            type="button"
            className="rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-800 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 dark:hover:bg-gray-800"
